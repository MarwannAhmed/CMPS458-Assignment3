...
from config import A2C_CONFIG, ENVIRONMENTS
...
def train_agent_on_env(
    ...
    """
    Train an A2C agent on a given environment.
    ...
    run = wandb.init(project="cmps458_assignment3", name=f"A2C-{env_name}", config={**config, "env_id": env_name, "agent": "A2C"})
    ...
    for episode in range(1, max_episodes + 1):
        ...
        mean_critic_loss = np.mean(step_critic_losses) if step_critic_losses else 0.0
        actor_losses_per_episode.append(mean_actor_loss)
        critic_losses_per_episode.append(mean_critic_loss)
        ...
        log(
            ...
            f"ActorLoss: {mean_actor_loss:.6f} | CriticLoss: {mean_critic_loss:.6f}"
        )
    ...
    training_stats = {
        ...
        "critic_losses": critic_losses_per_episode,
    }
...
# Plotting helper
# ------------------------------------------------------------------
def save_test_plots(env_name: str, test_durations, test_returns, figures_root="figures/A2C"):
    """
    Generate and save:
    - Episode duration vs episode
    - Episode return vs episode
    in figures/A2C/<env_name>
    """
    ...
    plt.title(f"A2C - {env_name} - Episode Duration vs Episode")
    plt.grid(True)
    ...
    plt.title(f"A2C - {env_name} - Episode Return vs Episode")
    plt.grid(True)
...
def main():
    ...
    # Iterate over all environments defined in ENVIRONMENTS / A2C_CONFIG
    for env_name, env_info in ENVIRONMENTS.items():
        if os.path.exists("trained_models/A2C/" + env_name + "/actor.pth") and os.path.exists("trained_models/A2C/" + env_name + "/critic.pth"):
            log(
                f"Trained models for {env_name} already exist. Skipping training."
            )
            continue
        if env_name not in A2C_CONFIG:
            log(
                f"Environment {env_name} has no A2C_CONFIG entry. Skipping."
            )
            continue

        max_episodes = A2C_CONFIG[env_name]["Training Episodes"]
        target_reward = env_info["target_reward"]
        config = A2C_CONFIG[env_name]
        ...
        if avg_test_return >= target_reward:
            ...
            # 2.c.1. Save models
            models_dir = os.path.join("trained_models", "A2C", env_name)
